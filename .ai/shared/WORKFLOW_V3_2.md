...
## Appendix A — Dormant TCW-053 composite protected-release contract (NOT ACTIVE)

**Status: NON-OPERATIVE / CONDITIONAL GOVERNANCE IMPLEMENTATION, NOT A RELEASE EXCEPTION.** This appendix is the narrow, task-specific TCW-053 delegation authorized by the owner for implementation and a fresh independent material-control audit only. It does not amend ordinary V3.2 TCW_TASK_V2 lifecycle, loosen Builder/Auditor scope or branch protection, install Actions, authorize a source/staging PR, merge, deployment, token/rights change or closeout. This appendix remains **DORMANT** pending separate owner staging and installation authorizations and independent executable audit.

The read-only offline validator scripts/workflow-composite-release.js defines schema TCW_COMPOSITE_RELEASE_ATTESTATION_V1 and three distinct identities: original audited Builder source A (preserving original task/PR/branch/checkpoint, true historical branch creation and separately approved effective scope baseline), independently audited Manager-owned stage S on a separate PR with frozen protected master M, and the actual protected merge G. A's original readiness packet and audit never certify S. S's SHA is held in an authenticated off-master immutable attestation/evidence commit, never embedded in S's own Git tree. Exactly four original source path/mode/blob tuples must match the entire staged file inventory. Any extra staged path, including Manager metadata, requires separate owner scope and independent audit, never an implicit exception.

Every future attempt needs a fresh high-entropy nonce, finite expiry, immutable A/S/M/ruleset/audit/owner tuple and protected independently verified durable append-only receipt ledger with PREPARED → REVIEWED → OWNER_APPROVED → RELEASED or irreversible ABORTED transitions. Only an authenticated protected receipt proves unused/consumed/aborted state: user-supplied JSON, mutable PR comments, moving refs, self-hashed packets and Git author names do not establish authority or nonce custody. Replay, expiry, superseded S, changed master/ref/ruleset/evidence or unverifiable ledger means **RELEASE HOLD**, new freeze and fresh approval. The offline module never writes/consumes the ledger.

Independently fetch and authenticate repository ID/name, A/S/M PRs/branches/commits/trees, complete stage diff and four source blobs, active strict ruleset, required exact-stage GitHub Actions FULL test from integration ID 15368 and synthetic required merge-preview check where applicable, independently authored whole-tree exact-S audit, off-master immutable Manager attestations, separate owner staging-plan and later exact-S installation approvals, and an authenticated rollback operator/plan. Verify allowed owner/Manager/Auditor identity, evidence publisher and actual permission from independently fetched protected records. Same-account chat roles, arbitrary signatures, checked boxes or unverified API status are not permission proof. Unavailable ledger, permissions, required check/preview or external authorization => **NOT VERIFIED / RELEASE HOLD**. Passing local fixtures proves only LOCAL_CONTRACT_PASS, not authenticated GitHub L4 or real staged release.

Future premerge is READ-ONLY: exclusively separately approved GitHub **merge** method, never squash/rebase/default/bypass. Immediately before an externally authorized protected merge, re-fetch live M/S/A/ruleset/test/preview/permissions/owner consent/ledger. GitHub expected_head_sha guards S, **NOT an atomic base-M compare-and-swap**. Drift, missing proof or 405 means STOP/new freeze; no bypass. After separately authorized actual merge, read-only postmerge must establish real G with exactly TWO ordered parents (parent1 frozen M, parent2 independently audited S), G.tree equal audited S.tree, live protected master equal G, actual merged full path/blob inventory, consumed-once protected ledger receipt and real FULL master CI/deploy/production evidence. Unexpected G is an incident, not permission for a force update or self-approved rollback.

The module provides read-only validate, premerge and postmerge CLI commands. Local validation may return LOCAL_CONTRACT_PASS only and never release readiness. Production premerge/postmerge remain RELEASE_HOLD whenever trusted identity/publication rights, durable ledger, exact required-check/preview, immutable off-master approval/audit and rollback cannot be proved. It has no merge/ref-update/write API, approval, ledger-writing, PR creation, staging, deployment, auto-freeze, token write or source closeout. Original Builder task registry/checkpoint and original helper, Manager-only audit freeze/merge authority, FULL CI and fresh independent audit remain unchanged. A new owner authorization is mandatory before any staging-source PR, another distinct exact-S authorization before installation; TCW-053 itself requires separately frozen fresh executable independent audit and normal master CI before any governance integration.
