...
test("start sit data confidence keeps completeness separate from freshness", () => {
  ...
  assert.equal(fresh.label, "High"); assert.equal(fresh.score, 100); assert.equal(fresh.completenessScore, 100); assert.equal(fresh.freshness, "fresh"); assert.deepEqual(fresh.limitations, []);

  const aging = assessStartSitDataConfidence({ meta: { capturedAt: "2026-09-01T12:00:00Z" } }, players, Date.parse("2026-09-01T13:00:00Z"));
  assert.equal(aging.label, "High"); assert.equal(aging.score, 94); assert.equal(aging.completenessScore, 100); assert.equal(aging.freshness, "aging"); assert.match(aging.limitations.join(" "), /Snapshot is aging/);
  ...
  assert.equal(partial.label, "Medium"); assert.equal(partial.score, 67); assert.equal(partial.completenessScore, 75); assert.equal(partial.freshness, "stale"); assert.match(partial.limitations.join(" "), /opponent unavailable.*kickoff unavailable.*stale/);
});
