...
import { DEFAULT_ESPN_CONNECTION, EspnConnectionPreferences, connectionKey, parseEspnTeamUrl } from "../src/providers/espn/connection-preferences.js";
...
test("ESPN team URLs resolve to numeric local connection settings", () => {
  const url = "https://fantasy.espn.com/football/team?leagueId=123456&seasonId=2026&teamId=7&fromTeamId=7";
  assert.deepEqual(parseEspnTeamUrl(url), { leagueId: "123456", seasonId: "2026", teamId: "7" });
  const preferences = new EspnConnectionPreferences({ storage: null });
  assert.deepEqual(preferences.save({ leagueId: url, seasonId: "", teamId: "" }), { leagueId: "123456", seasonId: "2026", teamId: "7" });
  assert.throws(() => parseEspnTeamUrl("https://example.com/football/team?leagueId=1&seasonId=2026&teamId=2"), /fantasy\.espn\.com/);
  assert.throws(() => parseEspnTeamUrl("https://fantasy.espn.com/football/team?leagueId=1&teamId=2"), /missing valid connection IDs/);
});
