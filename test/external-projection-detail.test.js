...
const set = { provider: "FantasyPros manual CSV", scoringFormat: "PPR", season: 2026, capturedAt: "2026-08-28T00:00:00Z", projections: [{ providerPlayerId: "fp-1", week: 1, points: 18.4 }, { providerPlayerId: "fp-2", week: 2, points: 15 }] };
...
test("player detail keeps mapped external values sourced and missing mappings explicit", () => {
  ...
  assert.match(ready.grid, /Week 1/); assert.match(ready.grid, /18\.4 pts/); assert.match(ready.grid, /Week 2/); assert.match(ready.grid, /Unavailable/);
  assert.match(ready.source, /FantasyPros manual CSV/); assert.match(ready.source, /1\/2 imported weeks/); assert.match(ready.source, /PPR/);
