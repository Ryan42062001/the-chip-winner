...
function describePriorWaiver(item) {
  if (item.kind === "ir-assisted-add") {
    return `Move ${item.irMove.player.name} from the bench to IR, then add ${item.add.name} with no drop`;
  }
  return `Add ${item.add.name} for ${item.drop.name}`;
}
...
export function diffWaiverRecommendations(previousSnapshot, currentSnapshot, teamId, now = snapshotTime(currentSnapshot, Date.now())) {
  ...
  for (const item of previous.items) {
    ...
    changes.push(Object.freeze({
      ...
      detail: `${describePriorWaiver(item)} was a +${item.lineupGain.toFixed(1)} current-week lineup recommendation at the prior capture. ${review.reason}`,
...
export function diffLineupRecommendations(previousSnapshot, currentSnapshot, teamId) {
  const previous = buildLineupSuggestions(previousSnapshot, teamId);
  const current = buildLineupSuggestions(currentSnapshot, teamId);
  const key = (item) => `${item.slot}:${item.sit.id}`;
  ...
  for (const [id, item] of before) {
    if (!after.has(id)) changes.push(Object.freeze({ kind: "recommendation", change: "cleared", title: `Suggestion cleared for ${item.sit.name}`, detail: `Starting ${item.start.name} is no longer recommended by the current projection threshold.`, playerId: item.sit.id }));
  }
