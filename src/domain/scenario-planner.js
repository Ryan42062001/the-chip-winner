...
export function buildScenarioPlan(snapshot, teamId, options = {}) {
  ...
  if (options.projectionSet && options.identityMap instanceof Map) {
    ...
    for (const scenario of options.scenarios || []) {
      ...
      const weekly = weeks.map((week) => {
        ...
        const deltaReady = result.projectedTotal != null && baseline != null && baselineEntry.completeCoverage && completeCoverage;
        const deltaUnavailableReason = deltaReady ? null : !baselineEntry?.completeCoverage ? "Baseline roster projection coverage is incomplete." : !completeCoverage ? "Scenario roster projection coverage is incomplete." : "A complete legal lineup total is unavailable.";
        return Object.freeze({ week, projectedTotal: result.projectedTotal ?? null, delta: deltaReady ? +(result.projectedTotal - baseline).toFixed(1) : null, deltaUnavailableReason, status: result.status, mappedProjectionCount, rosterPlayerCount: scenarioRosterIds.length, completeCoverage });
      });
  ...
  const coverage = Object.freeze({ completeWeeks: weeklyBaseline.filter((item) => item.completeCoverage).length, totalWeeks: weeklyBaseline.length, mappedProjectionCells, requiredProjectionCells, percentage: requiredProjectionCells ? Math.round((mappedProjectionCells / requiredProjectionCells) * 100) : 0 });
