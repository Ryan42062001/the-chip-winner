...
function renderSeasonPlan() {
  ...
  const horizon = plan.horizon.status === "available" ? `${plan.horizon.weeks.length} future week${plan.horizon.weeks.length === 1 ? "" : "s"} supplied` : "Future-week projections unavailable";
  const moves = buildRosterAwareWaiverIdeas(state.snapshot, state.selectedTeamId);
  const moveBody = moves.items?.length ? moves.items.slice(0, 3).map(item => `<div class="plan-row"><strong>${escapeHtml(item.add.name)} for ${escapeHtml(item.drop.name)}</strong><span>+${item.lineupGain} current-week lineup points · legal unlocked-bench simulation</span></div>`).join("") : `<p class="plan-note">${moves.status === "missing-availability" ? "ESPN availability is not included in this snapshot." : "No current-week add/drop scenario clears the action threshold."}</p>`;
  content.innerHTML = sectionHeader("Season Plan", "A transparent planning layer built from ESPN roster state and only explicitly reported FantasyPros context.") + `<div class="plan-grid"><div><div class="section-divider"><span>ROSTER DEPTH</span></div><div class="plan-groups">${depth}</div></div><div class="side-stack"><article class="panel"><div class="panel-head"><div><p class="eyebrow">BYE WEEKS</p><h3>Starter conflicts</h3></div></div>${bye}</article><article class="panel"><div class="panel-head"><div><p class="eyebrow">PLAYOFF CONTEXT</p><h3>Explicit schedule strength</h3></div></div>${playoff}</article><article class="panel"><div class="panel-head"><div><p class="eyebrow">CURRENT-WEEK SCENARIOS</p><h3>Top legal moves</h3></div></div>${moveBody}<a class="text-link" href="#waivers">Review all waiver scenarios →</a></article><article class="panel"><div class="panel-head"><div><p class="eyebrow">MULTIWEEK OUTLOOK</p><h3>${escapeHtml(horizon)}</h3></div></div><p class="plan-note">Future scenario comparisons stay disabled until ESPN or an approved projection source supplies future-week inputs.</p></article></div></div><article class="panel plan-limitations"><strong>Data boundaries</strong>${plan.limitations.map(item => `<span>${escapeHtml(item)}</span>`).join("")}</article>`;
}
