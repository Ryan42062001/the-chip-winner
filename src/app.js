...
function renderSeasonPlan() {
  ...
  const weeklyBody = scenarios.weeklyBaseline?.length ? scenarios.weeklyBaseline.map(item => { const gaps = gapDetails(item); return `<div class="plan-week"><div class="plan-row"><strong>Week ${item.week}</strong><span>${item.projectedTotal == null ? "No complete lineup" : `${item.projectedTotal.toFixed(1)} optimized known points`} · ${item.mappedProjectionCount}/${item.rosterPlayerCount} roster projections · ${item.completeCoverage ? "complete" : "partial"}</span></div>${gaps.length ? `<details class="coverage-details"><summary>Review ${gaps.length} missing input${gaps.length === 1 ? "" : "s"}</summary>${gaps.map((gap) => `<span>${escapeHtml(gap)}</span>`).join("")}</details>` : ""}</div>`; }).join("") : `<p class="plan-note">Future scenario comparisons need both weekly projections and the explicit ID mapping CSV.</p>`;
