const NORMALIZED_SET_CACHE = new WeakMap();
const PROJECTION_INDEX_CACHE = new WeakMap();

export function normalizeFutureProjectionSet(input) {
  if (input && typeof input === "object" && Object.isFrozen(input) && NORMALIZED_SET_CACHE.has(input)) {
    return NORMALIZED_SET_CACHE.get(input);
  }
  const errors = [];
  ...
  const value = Object.freeze({ provider: input.provider, scoringFormat: input.scoringFormat, season: input.season, capturedAt: oldestCapture, projections: Object.freeze(records) });
  const result = Object.freeze({ valid: true, errors: Object.freeze([]), value });
  NORMALIZED_SET_CACHE.set(value, result);
  if (Object.isFrozen(input)) NORMALIZED_SET_CACHE.set(input, result);
  return result;
}
...
export function indexFutureProjections(set) {
  if (set && typeof set === "object" && Object.isFrozen(set) && PROJECTION_INDEX_CACHE.has(set)) return PROJECTION_INDEX_CACHE.get(set);
  const index = new Map();
  for (const item of set?.projections || []) index.set(`${item.providerPlayerId}:${item.week}`, item.points);
  if (set && typeof set === "object" && Object.isFrozen(set)) PROJECTION_INDEX_CACHE.set(set, index);
  return index;
}
