...
export function normalizeEspnLeagueResponse(response, captureMeta = {}, supplemental = {}) {
  ...
  const scheduleIndex = indexNflSchedule(supplemental.nflScoreboard);
  ...
  const rosters = response.teams.map((team) => {
    const entries = (team.roster?.entries || []).map((entry) => {
      ...
      if (!playerMap.has(id)) playerMap.set(id, normalizeRosterPlayer(rawPlayer, currentWeek, scheduleIndex));
  ...
  for (const entry of availableEntries) {
    ...
    if (!playerMap.has(id)) playerMap.set(id, { ...normalizeRosterPlayer(player, currentWeek, scheduleIndex), availabilityStatus: entry.status || "AVAILABLE" });
  }
...
function normalizeRosterPlayer(player, currentWeek, scheduleIndex) {
  const projectionStat = (player.stats || []).find((stat) => stat.scoringPeriodId === currentWeek && stat.statSourceId === 1 && stat.statSplitTypeId === 1);
  const schedule = selectNflSchedule(player.proTeamId, scheduleIndex);
  return {
    id: String(player.id),
...
function indexNflSchedule(scoreboard) {
  if (!Array.isArray(scoreboard?.events) || scoreboard.events.length === 0) return null;
  const schedule = new Map();
  for (const event of scoreboard.events) {
    const competitors = event.competitions?.[0]?.competitors || [];
    for (const current of competitors) {
      const teamId = Number(current.team?.id);
      if (!Number.isFinite(teamId) || schedule.has(teamId)) continue;
      const opponent = competitors.find((item) => item !== current);
      schedule.set(teamId, Object.freeze({
        opponent: opponent ? ESPN_PRO_TEAMS[Number(opponent.team?.id)] || opponent.team?.abbreviation || null : null,
...
function selectNflSchedule(proTeamId, scheduleIndex) {
  if (!proTeamId || !scheduleIndex) return NO_SCHEDULE;
  return scheduleIndex.get(Number(proTeamId)) || BYE_SCHEDULE;
}
