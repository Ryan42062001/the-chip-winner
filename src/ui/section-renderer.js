...
let state, futureProjectionSet, projectionIdentityMap, selectedFutureWeeks, selectedPlayoffWeeks, savedEspnConnections, espnConnection, companionHealth, leagueScheduleWeek, projectionImportSummary;
...
const syncContext = () => ({ state, futureProjectionSet, projectionIdentityMap, selectedFutureWeeks, selectedPlayoffWeeks, savedEspnConnections, espnConnection, companionHealth, leagueScheduleWeek, projectionImportSummary } = deps.getContext());
...
const espnPlayoffWeeks = state.snapshot.league.playoffWeeks;
const playoffWeeks = selectedPlayoffWeeks || [];
const playoffBoundary = `<p class="plan-note"><strong>${espnPlayoffWeeks ? "ESPN league setting" : "Local browser setting"}:</strong> ${playoffWeeks.length ? `Weeks ${playoffWeeks.join(", ")}` : "Not configured"}. ${espnPlayoffWeeks ? "Read-only ESPN source." : "This does not change ESPN."}</p>${espnPlayoffWeeks ? "" : `<fieldset class="horizon-picker"><legend>Fantasy playoff weeks</legend><div class="horizon-options">${Array.from({ length: 18 }, (_, i) => i + 1).map((week) => `<label><input type="checkbox" data-playoff-week="${week}" ${playoffWeeks.includes(week) ? "checked" : ""}>Week ${week}</label>`).join("")}</div></fieldset>`}`;
...
const weeklyBody = scenarios.weeklyBaseline?.length ? `<div class="plan-scenario"><strong>Hold current roster · ${scenarios.baselineHorizonTotal == null ? "horizon total withheld" : `${scenarios.baselineHorizonTotal} selected-week points`}</strong></div>` + scenarios.weeklyBaseline.map(item => `<div class="plan-week"><div class="plan-row"><strong>Week ${item.week} · ${item.completeCoverage ? "usable" : "blocked"}</strong><span>${item.projectedTotal == null ? "No complete lineup" : `${item.projectedTotal.toFixed(1)} optimized known points`} · ${item.mappedProjectionCount}/${item.rosterPlayerCount}${item.completeCoverage ? " complete" : " · comparisons withheld"}</span></div>${item.starters?.length ? `<small>Starters: ${item.starters.map((starter) => `${escapeHtml(starter.slot)} ${escapeHtml(playerIndex.get(starter.playerId)?.name || starter.playerId)} (${starter.points} pts)`).join(" · ")}</small>` : ""}</div>`).join("") : `<p class="plan-note">Future comparisons need projections and explicit ID mappings.</p>`;
const futureMoves = scenarios.scenarios?.length ? scenarios.scenarios.map(item => `<div class="plan-scenario"><strong>Add ${escapeHtml(playerIndex.get(item.addPlayerId)?.name || item.addPlayerId)} · drop ${escapeHtml(playerIndex.get(item.dropPlayerId)?.name || item.dropPlayerId)}</strong><span>Selected horizon: ${item.horizonDelta == null ? escapeHtml(item.horizonUnavailableReason) : `${item.horizonDelta >= 0 ? "+" : ""}${item.horizonDelta} points versus hold`}</span>${item.weekly.map(week => `<span>Week ${week.week}: ${week.delta == null ? escapeHtml(week.deltaUnavailableReason) : `${week.delta >= 0 ? "+" : ""}${week.delta} points`}</span>`).join("")}</div>`).join("") : `<p class="plan-note">Complete mapped projections will evaluate legal waiver candidates.</p>`;
