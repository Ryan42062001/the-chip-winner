...
Status: **In progress.** Validated browser-local ESPN league, season, and team configuration, complete browser-local deletion, best-effort mobile-sync revocation, the first Content Security Policy, skip navigation, section focus management, mobile-menu semantics, and reduced-motion support are implemented. Multiple saved leagues, cache migrations, production smoke automation, and formal accessibility/security audits remain open.
...
- Add connection health, extension version compatibility, refresh cooldown, and recovery guidance.
