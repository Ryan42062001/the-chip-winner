...
`npm run eval:model` executes twelve versioned recommendation fixtures and six adversarial explanation fixtures, and the deployment workflow requires all of them to pass. Runtime recommendation validation mirrors the JSON Schema boundary, including timestamp, payload, and additional-field checks. Explanation output is separately evaluated for the correct recommendation ID, provider attribution, named source inputs, every stated limitation, strict metadata, and unexpected fields. Per-recommendation failures are returned without exposing credentials or aborting unrelated explanations.
