...
`npm run eval:model` executes thirteen versioned recommendation fixtures and seven adversarial explanation fixtures, and the deployment workflow requires all of them to pass. Runtime recommendation validation mirrors the JSON Schema boundary, including timestamp, payload, and additional-field checks. A drop proposal is blocked when selected-team context is absent because bench and lock legality cannot be verified. Explanation output is separately evaluated for the correct recommendation ID, provider attribution, named source inputs, every stated limitation, strict metadata, unexpected fields, and invalid response shapes. Per-recommendation failures are returned without exposing credentials or aborting unrelated explanations.
