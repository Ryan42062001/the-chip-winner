...
- 134 automated tests covering league normalization across regular, superflex, offseason, playoff and partial-live fixtures, full reported matchup schedules and locally sorted record overviews, recommendations, identity reconciliation, encrypted sync, snapshot differencing, lineup locks, roster-aware waiver simulation, recommendation-change explanations, season planning, persisted horizons and projection coverage, persistent prioritized alerts, accessible first-run onboarding, multiple local ESPN connections, companion compatibility and cooldowns, versioned cache migrations and deletion, recommendation contracts, privacy-safe model context, guarded model adapters, recommendation and explanation evaluation, and future projection inputs.
