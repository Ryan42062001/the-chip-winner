...
- 260 automated permanent tests plus 21 deployment-blocking model safety fixtures covering league normalization, lineup and waiver legality, ESPN IR eligibility/roster validity, IR-assisted no-drop waiver paths, acquisition limits, identity reconciliation, projection imports, provider-ID supersession, zero-cost DynastyProcess staging, classified unresolved rows, encrypted sync, snapshot differencing, season planning, accessibility, browser behavior, local-data deletion, and the guarded one-click weekly projection update workflow.
...
## Current execution plan — after v0.9.64
...
A temporary live Chrome canary validated the actual browser path before release: the current source and publication endpoints returned HTTP 200, the control became **Update Week 1 projections**, the click imported 648 Week 1 projection records through the normal caches, and the receipt recorded 648/682 with 34 unresolved. The temporary canary was removed; the current permanent suite is 260 tests.
...
IR-assisted advice is revalidated as a two-step plan after ESPN refreshes. Lost IR capacity, a changed injury designation, a lock, changed availability, exhausted acquisition capacity, roster/position-limit changes, or insufficient projected gain can make the prior plan obsolete; unsupported status evidence keeps it unverified. The **What Changed** timeline preserves the explicit IR move and never invents a drop. Season Plan deliberately excludes IR-assisted recommendations from its existing add/drop-only multiweek planner until that planner can retain the injured player in IR across future weeks and prove complete player-week coverage for both active and IR-retained roster states.
...
2. If IR-retained multiweek scenarios are added, extend the future-roster model explicitly rather than reusing add/drop assumptions; require full coverage for every retained active and IR player across the selected horizon.

Acceptance criteria: every current-week move remains legal after full-roster simulation; IR-assisted paths prove both the bench-to-IR step and the subsequent acquisition while retaining `drop: null`; current invalid IR state blocks acquisition advice while unsupported IR state remains unverified; prior advice that no longer satisfies the latest supported state is marked obsolete; unsupported acquisition likelihood is never claimed; current-week, replacement-value, ROS ranking, IR legality, and multiweek conclusions remain separately sourced.
