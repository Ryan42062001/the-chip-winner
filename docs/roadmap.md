...
## Current execution plan — after v0.9.60
...
Status: **Partially implemented through v0.9.60.** Season Plan renders an exact roster-and-ESPN-candidate coverage matrix across the selected horizon. Candidate-aware future coverage checks the top current ESPN waiver adds and distinguishes missing explicit identity mappings from missing player-week projections. The zero-cost local DynastyProcess stager retains the source-published PPR `r2p_pts` estimate, requires the NFL week explicitly because the source file omits it, records GitHub publication provenance, and emits the existing projection/identity contracts into ignored `local-data/`. Normal athletes prefer DynastyProcess's published FantasyPros-to-ESPN crosswalk; D/ST uses the explicit source-team-to-ESPN synthetic-ID bridge added in v0.9.58. v0.9.59 added ten reviewed athlete bridges for the narrow upstream condition where the weekly feed supplies a FantasyPros ID and the current DynastyProcess ID table independently exposes the target ESPN ID with no FantasyPros assignment. v0.9.60 adds explicit provider-ID supersession semantics for reviewed rollover cases: the replacement identity-map row names `supersedes_provider_player_id`, the occupied ESPN identity must already belong to that exact predecessor when merging into an existing cache, and the predecessor remains mapped so historical projection weeks continue to resolve. Roman Wilson now maps current FantasyPros ID `28896` and historical FantasyPros ID `26160` to ESPN `4431492` only while current DynastyProcess evidence still maps the predecessor to that exact ESPN identity; a future direct mapping for `28896` automatically supersedes the reviewed rule. Ordinary duplicate ESPN mappings still fail. Display names are never runtime joins. Under the verified 2026-09-04 Week 1 source state, coverage improves from 647/682 to **648/682 mapped (95.01%)**, leaving 34 unresolved: QB 1, RB 10, WR 8, TE 13, K 2. D/ST remains 32/32 and kicker coverage remains 32/34. Stale-team or bad third-party rows remain excluded rather than forced into current ESPN identities. Real multiweek coverage still depends on accumulating each actually published week; no future week is fabricated or inferred.
...
5. For future provider-ID rollovers, require an explicit reviewed predecessor-to-replacement relationship and exact stable ESPN identity evidence; do not weaken duplicate-ID protections generically.
...
Acceptance criteria: every used value includes provider, scoring, season, explicitly assigned week, provider ID, points, and capture/publication provenance; incompatible scoring blocks comparisons; missing, stale-team, or ambiguous IDs remain excluded; reviewed bridges and supersessions require exact current independent upstream identity evidence and fail closed when that evidence changes; ordinary duplicate ESPN mappings remain conflicts; historical provider IDs remain available for prior imported weeks; partial coverage never produces a summed advantage; third-party weekly source data is not bundled into the public site.
