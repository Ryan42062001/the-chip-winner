...
- 118 automated tests covering league normalization across regular, superflex, offseason, playoff and partial-live fixtures, recommendations, identity reconciliation, encrypted sync, snapshot differencing, lineup locks, roster-aware waiver simulation, recommendation-change explanations, season planning, persistent prioritized alerts, accessibility markup, multiple local ESPN connections, companion compatibility and cooldowns, versioned cache migrations and deletion, recommendation contracts, privacy-safe model context, guarded model adapters, model-output evaluation, and future projection inputs.
