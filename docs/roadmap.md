...
- 263 automated permanent tests plus 21 deployment-blocking model safety fixtures covering league normalization, lineup and waiver legality, ESPN IR eligibility/roster validity, IR-assisted no-drop waiver paths, multiweek IR-retained scenario safety, acquisition limits, identity reconciliation, projection imports, provider-ID supersession, zero-cost DynastyProcess staging, classified unresolved rows, encrypted sync, snapshot differencing, season planning, accessibility, browser behavior, local-data deletion, and the guarded one-click weekly projection update workflow.
...
## Current execution plan — after v0.9.65
...
v0.9.65 extends the multiweek scenario planner and Season Plan for those same explicit IR-assisted paths. A future IR simulation first proves the matching current ESPN-derived `ir-assisted-add` recommendation still exists, moves only the supported unlocked bench player into IR, and adds the ESPN-available target without a drop. The simulated roster retains both players, so projection coverage is evaluated across the full roster state rather than pretending the injured player disappeared. Weekly and horizon deltas are withheld when either the baseline or simulated roster lacks any required player-week value. Season Plan now labels current-week and future IR paths as **move to IR · no drop** and no longer assumes every waiver scenario contains `drop` data.

Remaining work:

1. Accumulate real multiweek source publications and expose waiver impact only for selected weeks where both baseline and simulated rosters have complete mapped player-week coverage.
2. Continue validating IR-assisted future behavior against materially different ESPN roster states, injury transitions, locks, acquisition limits, roster/position limits, and future projection gaps without broadening the fail-closed eligibility policy.

Acceptance criteria: every current-week move remains legal after full-roster simulation; IR-assisted paths prove both the bench-to-IR step and the subsequent acquisition while retaining `drop: null`; multiweek IR-assisted paths independently match current ESPN legality, retain the IR player, and require complete projection coverage before any delta is exposed; current invalid IR state blocks acquisition advice while unsupported IR state remains unverified; prior advice that no longer satisfies the latest supported state is marked obsolete; unsupported acquisition likelihood is never claimed; current-week, replacement-value, ROS ranking, IR legality, and multiweek conclusions remain separately sourced.
