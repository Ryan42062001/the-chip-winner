...
- 283 automated permanent tests plus 21 deployment-blocking model safety fixtures covering league normalization, lineup and waiver legality, ESPN IR eligibility/roster validity, IR-assisted no-drop waiver paths, multiweek IR-retained scenario safety, future add/drop legality transitions, transparent waiver priority bands, acquisition limits, identity reconciliation, projection imports, provider-ID supersession, zero-cost DynastyProcess staging, classified unresolved rows, encrypted sync, snapshot differencing, season planning, accessibility, browser behavior, local-data deletion, and the guarded one-click weekly projection update workflow.
...
- a transparent Waiver Wire priority board that keeps current-week gain, complete-horizon future gain, positive future-week coverage, replacement value, same-position depth context, and no-drop roster preservation separate while assigning non-dominated Pareto priority bands instead of a hidden weighted score;
...
## Current execution plan — after v0.9.67
...
v0.9.66 closes the corresponding legality gap for ordinary future add/drop inputs. Before a multiweek add/drop is simulated, the planner now verifies that ESPN still reports the add available, both add and drop are unlocked at one shared evaluation time, the drop remains on the selected bench, acquisition capacity is not proven exhausted, the current IR roster state is supported, and the resulting roster obeys every explicit ESPN size and provider-position limit. This legality gate is intentionally separate from the current-week action threshold: a move that is not useful this week may still be evaluated for future value when its current ESPN transaction state is legal. Unsupported future scenario kinds fail closed rather than defaulting to add/drop. IR-assisted future paths retain the stricter current matching-recommendation requirement because their two-step eligibility/capacity semantics are current-state dependent.

v0.9.67 adds a transparent priority layer over the current ESPN-legal recommendation set. `src/domain/waiver-priority-engine.js` combines no hidden weighted score: it keeps current-week lineup gain, complete selected-horizon future gain, the share of completely projected future weeks with a positive lineup delta, same-position replacement value, exact current same-position roster depth context, and IR/no-drop roster preservation as separate factors. Candidates are assigned Pareto priority bands. A move only enters a better band when it is no worse on every fully comparable known factor and better on at least one; genuine short-term-versus-future tradeoffs can therefore remain together in priority band 1. Missing future or replacement evidence remains missing and cannot count as zero or an advantage. Exact same-position depth is descriptive context only, not an invented universal roster-need threshold. Waiver Wire renders these factors and uses the same application-owned snapshot, future projection set, identity map, and selected planning horizon as Season Plan.

Remaining work:

1. Expand candidate discovery to **future-only legal waiver moves** that may not clear the current-week 0.5-point action threshold. Apply the same current ESPN legality gates before simulation and require complete baseline plus simulated-roster player-week coverage before exposing future value. Feed qualifying moves into the same transparent priority factors rather than creating a second score.
2. Accumulate real multiweek source publications and expose waiver impact only for selected weeks where both baseline and simulated rosters have complete mapped player-week coverage.
3. Validate ordinary add/drop, IR-assisted, and priority-band behavior against materially different authenticated ESPN league states, especially custom acquisition caps, position limits, IR roster states, and game-lock transitions, without weakening fail-closed behavior.

Acceptance criteria: every current-week move remains legal after full-roster simulation; ordinary multiweek add/drop paths may be future-positive without a current-week gain but must pass current ESPN availability, lock, acquisition, IR-roster, size, and provider-position legality before simulation; IR-assisted paths prove both the bench-to-IR step and the subsequent acquisition while retaining `drop: null`; multiweek IR-assisted paths independently match current ESPN legality, retain the IR player, and require complete projection coverage before any delta is exposed; current invalid IR state blocks acquisition advice while unsupported IR state remains unverified; unknown future transaction kinds fail closed; priority never hides factors behind a composite score, never converts missing evidence to zero, and treats band 1 only as non-dominated among the evaluated legal candidates; prior advice that no longer satisfies the latest supported state is marked obsolete; unsupported acquisition likelihood is never claimed; current-week, replacement-value, ROS ranking, IR legality, priority comparison, and multiweek conclusions remain separately sourced.
