...
## Current execution plan — after v0.9.59
...
Status: **Partially implemented through v0.9.59.** Season Plan renders an exact roster-and-ESPN-candidate coverage matrix across the selected horizon. Candidate-aware future coverage checks the top current ESPN waiver adds and distinguishes missing explicit identity mappings from missing player-week projections. The zero-cost local DynastyProcess stager retains the source-published PPR `r2p_pts` estimate, requires the NFL week explicitly because the source file omits it, records GitHub publication provenance, and emits the existing projection/identity contracts into ignored `local-data/`. Normal athletes prefer DynastyProcess's published FantasyPros-to-ESPN crosswalk; D/ST uses the explicit source-team-to-ESPN synthetic-ID bridge added in v0.9.58. v0.9.59 adds ten reviewed athlete bridges for a narrow upstream condition where the weekly feed supplies a FantasyPros ID and the current DynastyProcess ID table independently exposes the target ESPN ID with no FantasyPros assignment. Those bridges remain active only while that exact evidence persists, fail closed if the target ESPN ID becomes claimed, and yield automatically to a future direct upstream mapping. Display names are never runtime joins. Live Week 1 validation on the 2026-09-04 source improves from 637/682 to **647/682 mapped (94.87%)**, leaving 35 unresolved: QB 1, RB 10, WR 9, TE 13, K 2. D/ST remains 32/32 and kicker coverage improves from 24/34 to 32/34. Roman Wilson remains intentionally unresolved as a true FantasyPros provider-ID rollover/alias collision; stale-team or bad third-party rows remain excluded rather than forced into current ESPN identities. Real multiweek coverage still depends on accumulating each actually published week; no future week is fabricated or inferred.
...
5. Treat provider-ID rollover/alias cases as a separate identity-supersession design problem; do not weaken one-to-one collision protection to force them through.
...
Acceptance criteria: every used value includes provider, scoring, season, explicitly assigned week, provider ID, points, and capture/publication provenance; incompatible scoring blocks comparisons; missing, stale-team, ambiguous, or colliding IDs remain excluded; reviewed bridges require current independent upstream ESPN-ID evidence and fail closed when that evidence changes; partial coverage never produces a summed advantage; third-party weekly source data is not bundled into the public site.
