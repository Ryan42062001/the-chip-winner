...
- Preserve mobile navigation, keyboard behavior, accessibility, CSP, cache migrations, companion least privilege, and performance budgets.
- Do not raise performance budgets merely to make a check pass. If UI work expands `src/ui/section-renderer.js`, prefer cohesive view-level extraction rather than adding more application state there.
