...
Checkpoint: protected master; application release v0.9.59 extends the zero-cost DynastyProcess weekly PPR staging path with fail-closed reviewed identity bridges for ten current upstream FantasyPros-to-ESPN gaps. D/ST continues to use the source-published NFL team code plus an explicit ESPN pro-team-ID table to derive ESPN's synthetic team-defense player ID. Normal athletes still prefer DynastyProcess's direct stable crosswalk; a reviewed bridge activates only when the current DynastyProcess player-ID table independently exposes the reviewed ESPN ID on an otherwise-unassigned FantasyPros row. Do not rely on a SHA copied into this handoff: fetch origin/master and verify the actual tip before editing. The verified release baseline has 230 automated tests and 21 model-safety fixtures, and the GitHub Pages workflow must pass deployment and production smoke verification.
...
- The zero-cost command `npm run projections:dynastyprocess-weekly -- --season <year> --week <1-18>` stages the current DynastyProcess FantasyPros-derived weekly PPR `r2p_pts` signal into ignored local-data projection/identity CSVs and a provenance sidecar. The upstream weekly file has no NFL week column, so week must remain explicit. Normal athletes use the published stable FantasyPros-to-ESPN ID crosswalk whenever present. D/ST rows use the source-published team code plus an explicit ESPN pro-team table to derive ESPN's synthetic defense ID; unknown team codes and conflicting direct IDs fail closed. v0.9.59 adds ten audited athlete bridges where the weekly feed publishes a FantasyPros ID while the current DynastyProcess player-ID table independently publishes the target ESPN ID with no FantasyPros assignment. Each bridge remains active only while that exact unassigned ESPN evidence persists, fails closed if the ESPN ID becomes claimed by another FantasyPros ID, and automatically yields to a future direct upstream mapping. Display names are never used as runtime identity joins.
- Live Week 1 validation on the 2026-09-04 source now maps 647/682 rows overall. D/ST is 32/32. Kicker coverage improved from 24/34 to 32/34; the two remaining kicker rows are Charlie Smyth with stale external-team state relative to ESPN and the clearly stale/bad Andrew Wellock source row. The remaining unresolved set is 35: QB 1, RB 10, WR 9, TE 13, K 2.
- Roman Wilson remains intentionally unresolved because the weekly feed now uses FantasyPros ID 28896 while DynastyProcess still maps ESPN 4431492 to older FantasyPros ID 26160. This is a provider-ID rollover/alias case and must not be forced through the current one-to-one identity map without explicit supersession semantics.
- Exact ESPN identities also exist for Michael Trigg, Cam Grandy, and Charlie Smyth, but ESPN currently reports them without the source's claimed NFL team affiliation, so their weekly projections remain excluded rather than treating stale team data as current.
