...
- Waiver priority is transparent and comparative, not a hidden predictive score. v0.9.67 assigns Pareto priority bands only across current-week ESPN-legal waiver recommendations. A move may outrank another only when it is no worse on every fully comparable known factor and better on at least one; conflicting current-week versus future-week tradeoffs remain in the same non-dominated band. Keep current-week lineup gain, complete-horizon future gain, positive future-week rate, replacement value, same-position roster depth context, and IR/no-drop preservation separately inspectable. Missing future or replacement evidence stays missing and cannot become zero or an advantage. Exact same-position depth is context only; do not invent universal positional-need thresholds. Broader future-only candidate discovery below the current-week action threshold is a separate follow-up and must not be implied by the v0.9.67 board.
...
2. Finish the remaining Waiver Engine v2 work with projection-gated multiweek impact only after coverage gates pass. Current-week ESPN IR eligibility/roster-validity, explicit IR-assisted no-drop recommendations, fail-closed multiweek IR-retained Season Plan modeling, current-legality hardening for explicit future add/drop scenarios, and the transparent Pareto-band waiver priority board are implemented. Preserve the distinction between legality, current-week utility, future utility, replacement value, roster-depth context, and roster preservation; next expand discovery to legal future-only candidates without inventing value for missing weeks.
