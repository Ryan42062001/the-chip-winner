...
- Do not feed IR-assisted no-drop recommendations into the add/drop-only multiweek scenario planner until that planner can retain the IR player across future weeks and satisfy complete player-week coverage for both active and IR-retained roster states.
...
2. Finish the remaining Waiver Engine v2 work with projection-gated multiweek impact only after coverage gates pass. Current-week ESPN IR eligibility/roster-validity and explicit IR-assisted no-drop recommendations are implemented; keep both fail-closed and read-only, and keep IR-assisted moves outside the add/drop-only multiweek planner until that planner is extended deliberately.
