...
2. Finish season/playoff intelligence with a documented, approved position-specific strength-of-schedule source and methodology before displaying difficulty grades.
3. Complete production-readiness closeout: manual accessibility, companion threat review, recovery/deletion checks, and materially different live ESPN league states. Use those authenticated states as field validation for the already-complete Waiver Engine v2 rather than reopening its deterministic implementation unless a reproduced defect appears.
...
Waiver Engine v2 is complete in v0.9.69. Future-only IR-assisted stash discovery is a separately gated policy enhancement, not unfinished v2 work. Trade analysis, external notifications, server-side models, and ESPN write actions remain gated by the roadmap. Do not pull them forward merely because their interfaces or schemas already exist.