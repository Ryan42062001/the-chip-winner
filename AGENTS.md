...
1. Complete real multiweek projection coverage using the zero-cost local DynastyProcess weekly PPR staging path where source rows and stable identity evidence are available. Keep exact missing player-week and identity gaps visible and classify already-unresolved weekly rows without weakening identity rules. Preserve the explicit D/ST bridge, fail-closed reviewed identity bridges, explicit provider-ID supersession semantics, reviewed stale-source exclusions, and the classification-only ESPN Fantasy diagnostic boundary. New rollover cases require reviewed predecessor→replacement evidence; never relax duplicate-ID checks generically or convert diagnostic display matching into an identity join.
