...
try {
  ...
  for (const expected of ["The Chip Winner", "Content-Security-Policy", "src/app.js?v=0.9.20", "src/styles.css?v=0.9.20"]) if (!html.includes(expected)) throw new Error(`HTML smoke check missing ${expected}.`);
  for (const path of ["/src/app.js?v=0.9.20", "/src/styles.css?v=0.9.20", "/src/data/sample-espn-snapshot.json", "/schema/espn-snapshot.schema.json", "/schema/model-evaluation-report.schema.json"]) { const response = await fetch(`${origin}${path}`); if (!response.ok) throw new Error(`Static asset failed: ${path}.`); }
