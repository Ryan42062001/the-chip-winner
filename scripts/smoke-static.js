...
try {
  ...
  for (const expected of ["The Chip Winner", "Content-Security-Policy", `src/app.js?v=${assetVersion}`, `src/styles.css?v=${assetVersion}`, `src/zoom-reflow.css?v=${assetVersion}`]) if (!html.includes(expected)) throw new Error(`HTML smoke check missing ${expected}.`);
  for (const path of [`/src/app.js?v=${assetVersion}`, `/src/styles.css?v=${assetVersion}`, `/src/zoom-reflow.css?v=${assetVersion}`, "/src/data/sample-espn-snapshot.json", "/schema/espn-snapshot.schema.json", "/schema/model-explanation.schema.json", "/schema/model-evaluation-report.schema.json"]) { const response = await fetch(`${origin}${path}`); if (!response.ok) throw new Error(`Static asset failed: ${path}.`); }
